...
namespace AC2RE.Tools {
    public class HeightMap {
        private const int LANDBLOCK_COUNT = 255;
        private const int BLOCK_SIZE = 17;
        private const int CELL_SIZE = 10;
        private const int GLOBAL_SIZE = LANDBLOCK_COUNT * BLOCK_SIZE * CELL_SIZE;
        ...
        public Vector3[,] GeneratePositions() {
            var points = new Vector3[LANDBLOCK_COUNT * (BLOCK_SIZE - 1), LANDBLOCK_COUNT * (BLOCK_SIZE - 1)];

            for (byte landy = 0; landy < LANDBLOCK_COUNT; landy++) {
                for (byte landx = 0; landx < LANDBLOCK_COUNT; landx++) {
        ...
        private Vector3 CreateVectorPoint(CLandBlockData landBlockData, byte landx, byte landy, int x, int y) {
            int tmpx = (landx * BLOCK_SIZE + x) * CELL_SIZE;
            int tmpy = GLOBAL_SIZE - ((landy * BLOCK_SIZE + y) * CELL_SIZE) - 1;
            int tmpz = landBlockData.heights[x * BLOCK_SIZE + y];
